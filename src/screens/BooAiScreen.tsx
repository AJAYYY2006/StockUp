...
export default function BooAiScreen() {
    ...
    <div className="flex flex-col items-center text-center pt-8 pb-6 px-4">
      <div className="relative mb-4">
        <motion.div
          ...
          className="w-24 h-24 bg-[#5F714B] rounded-[2rem] rounded-tr-[4rem] rounded-bl-[4rem] shadow-xl flex items-center justify-center relative border-4 border-white/10"
        >
          <Leaf size={40} strokeWidth={1.5} className="text-[#CFC3A7]" />
          <motion.div
            animate={{ scale: [1, 1.2, 1], rotate: [0, 20, -20, 0] }}
            transition={{ repeat: Infinity, duration: 3, delay: 0.5 }}
            className="absolute -top-1 -right-1 bg-white p-1.5 rounded-full shadow-lg"
          >
            <Sparkles size={14} className="text-[#5F714B]" />
          </motion.div>
        </motion.div>
      </div>
      <h1 className="text-3xl font-black text-[#5F714B] tracking-tighter">{t('boo.title', 'BOO AI')}</h1>
      <p className="font-bold text-[#9BA88D] mt-1 text-sm tracking-wide uppercase opacity-80">{t('boo.subtitle', 'Your smart store assistant')}</p>
    </div>
  );
  ...
  const InsightCards = () => (
    <div className="flex flex-col gap-3">
      <div className="flex items-center justify-between px-1">
        <h2 className="text-xs font-black text-[#95A07A] uppercase tracking-[0.2em]">{t('boo.liveInsights', 'Live Insights')}</h2>
        {!isPro && <span className="text-[10px] font-black bg-white/50 text-[#95A07A] px-2 py-0.5 rounded-full border border-[#95A07A]/10">LOCKED</span>}
      </div>
      <AnimatePresence>
        {insightsToShow.map((insight, idx) => (
          <motion.div key={insight.id} initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: idx * 0.08 }}>
            <Card className="bg-[#5F714B] border-none text-[#F8F3E5] p-4 shadow-lg relative overflow-hidden group hover:scale-[1.01] transition-transform h-full">
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <div className="p-1.5 rounded-xl" style={{ backgroundColor: insight.accent }}>
                      <insight.icon size={15} className="text-[#CFC3A7]" />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-[0.15em] text-[#CFC3A7]">{insight.category}</span>
                  </div>
                  <ArrowRight size={13} className="text-[#CFC3A7]/40 group-hover:text-[#CFC3A7] transition-colors" />
                </div>
  ...
  const ChatHistory = () => (
    <div className="flex flex-col gap-4">
      <h3 className="text-xs font-black text-[#95A07A] uppercase tracking-[0.2em] px-1">{t('boo.recentThinking', 'Recent Thinking')}</h3>
      {MOCK_HISTORY.map((chat, idx) => (
        ...
        >
          <span className="text-[10px] font-black text-[#9BA88D] uppercase tracking-widest px-1">
          ...
          <div className={cn("px-4 py-3 rounded-2xl text-sm font-medium leading-normal shadow-sm",
            chat.role === 'user' ? "bg-[#95A07A] text-white rounded-tr-none" : "bg-white text-[#5F714B] rounded-tl-none border border-[#CFC3A7]/30"
          )}>
  ...
  const ChatInput = ({ className }: { className?: string }) => (
    <form onSubmit={handleSend} className={cn("bg-white rounded-[20px] p-2 shadow-lg flex items-center gap-2 border border-[#F8F3E5]", className)}>
      <div className="w-9 h-9 bg-[#F8F3E5]/50 rounded-xl flex items-center justify-center shrink-0">
        <Sparkles size={16} className="text-[#95A07A]" />
      </div>
      <input
        type="text"
        value={question}
        ...
        className="flex-1 bg-transparent border-none outline-none text-sm font-bold placeholder-[#95A07A]/40 py-1"
      />
      <button
        type="submit"
        disabled={!question.trim()}
        className="w-9 h-9 bg-[#5F714B] text-white rounded-xl flex items-center justify-center shrink-0 shadow-md disabled:opacity-30 hover:bg-[#95A07A] transition-all active:scale-95"
      >
        <Send size={16} />
      </button>
    </form>
  ...
  return (
    <PageTransition className="flex flex-col md:flex-row h-full min-h-screen bg-[#F8F3E5] text-[#5F714B] overflow-hidden">
      ...
      <div className="hidden md:flex flex-col flex-1 min-w-0 border-r border-[#CFC3A7]/40 overflow-hidden">
        <AvatarHeader />
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar px-6 pb-4">
          {isPro ? <ChatHistory /> : (
            <div className="flex flex-col items-center justify-center h-full py-12 text-center">
              <Sparkles size={36} className="text-[#CFC3A7] mb-4" />
              <p className="text-sm font-black text-[#9BA88D]">Upgrade to Pro to chat with BOO AI</p>
            </div>
          )}
        </div>
        ...
        {isPro && (
          <div className="shrink-0 p-4 border-t border-[#CFC3A7]/30">
            <ChatInput />
          </div>
        )}
      </div>
